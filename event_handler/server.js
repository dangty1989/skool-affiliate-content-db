...
async function summarizeJob(results) {
  try {
    ...
    // v1.2.72-beta.7: Detect missing system prompt
    if (!systemPrompt || systemPrompt.trim().length === 0) {
      console.error('[summarizeJob] ERROR: JOB_SUMMARY.md is missing or empty. Check operating_system/JOB_SUMMARY.md exists and is readable.');
    }
    ...
    // v1.2.72-beta.7: Enhanced job summary logging
    console.log(`[summarizeJob] System prompt length: ${systemPrompt.length} chars`);
    console.log(`[summarizeJob] User message length: ${userMessage.length} chars`);
    ...
    const summary = (result.content?.[0]?.text || '').trim() || 'Job completed.';

    // v1.2.72-beta.7: Log result preview for debugging
    console.log(`[summarizeJob] Result preview: ${summary.substring(0, 150)}...`);
